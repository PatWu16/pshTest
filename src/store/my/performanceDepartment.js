 ...
 */
import { getDepartmentStatisticsList } from '../../servers/my'
...
const mutations = {
  saveDepartmentStatisticsList (state, payload) {     // 存储客户统计列表数据
    state.departmentStatisticsList = payload
  }
}

const actions = {
  // 获取部门统计列表
  getDepartmentStatisticsList ({commit, state, dispatch, getters}, payload) {
    getDepartmentStatisticsList(payload.time, payload.id).then((res) => {
      // 过滤整合列表
      let filterList = []
      res.info.map(function (item) {
        ...
        if (len) {
          filterList.map(function (i, index) {
            if (i.memberId === item.empid_业务员ID) {
              i.statisticsList.push(item)
              return
            }
            if (index === (filterList.length - 1)) {
              filterList.push({
                memberId: item.empid_业务员ID,
                statisticsList: [item]
              })
            }
          })
      ...
      const result = filterList.map(function (item, index) {
        let resultItem = {
          itemHeader: [{
            label: '业务员',
            value: item.statisticsList[0].empnm_业务员
          }, {
            label: '销售目标',
            value: '-'
          }, {
          ...
          isShowDetail: false,
          memberId: item.statisticsList[0].empid_业务员ID,
          memberName: item.statisticsList[0].empnm_业务员
        }
      ...
      commit('saveDepartmentStatisticsList', result)
    })
  }
}
